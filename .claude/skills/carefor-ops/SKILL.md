...
- **제외번호**: 앱계정(minoneki@caring.co.kr) 소유 구글시트 `충청본부_상담제외번호(자동관리)`(consult_report.EXCL_SSID). 앱이 Drive media PATCH로 쓰기(덮어쓰기), 웹앱 ssid로 읽기. 공용 모듈 `excl_store.py`(read_rows/add_phones). **본사 시트(주보/상담시트)는 읽기전용·수정금지.** 본사 웹앱은 `spreadsheets.readonly`라 거기에 쓰기 붙이면 재인증→읽기 깨짐(금지).
  - 추가 3경로: ① `exclude_tool.py`(미입력 목록 선택) ② 제외시트 직접 타이핑 ③ **지점 상담공지 엑셀 맨 앞 '제외(O)' 열에 지점이 체크** → publish_excel의 `harvest_exclude_marks`가 재생성 전 수집해 자동 등록. 이 때문에 **지점 엑셀은 anyone writer**(연락처 포함이라 보안 주의 — 지점 이메일 한정 공유로 조일 수 있음).
- **매일 자동 발송**(consult_report.yml, 예약 KST 08:00). 발송 본체는 **`publish_excel.py`** — 지점 '제외 ✔' 수집 → 엑셀 재생성 → 드라이브 덮어쓰기 → **표 + 지점별 엑셀 링크**를 슬랙 발송. 실패 시 `consult_report.py`(링크 없음)로 폴백해 공지는 반드시 나감. 워크플로에 **`pip install openpyxl` 필수** (2026-07-10까지 이게 없어 publish_excel이 매일 ModuleNotFoundError로 죽고 있었음 — 지점 엑셀 미갱신·'제외 ✔' 미수집).
- ⚠️ **`continue-on-error: true` 스텝은 실패해도 GitHub API가 `conclusion=success`로 보고한다.** 성공 여부는 반드시 런 로그(outcome)를 열어 확인할 것 — API 값만 믿고 "성공"이라 오보고한 사례 있음(2026-07-10).
- ⚠️ **GitHub `schedule` cron은 9~12시간까지 지연된다.** 정시 발송이 필요하면 cron-job.org에 workflow_dispatch 호출 job 등록 필수. 2026-07-10 기준 **출석관리만 등록됨, 상담 공지는 미등록**(그래서 예약 08:00이어도 오후~저녁 도착).
